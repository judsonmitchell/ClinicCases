...
<table width='70%' align=left border=0 style='margin-left:30px;margin-top:30px;'><tr><td valign='top'>";

while ($r = mysql_fetch_array($query))
{
$namer = getNameAsVar2($r[username]);
echo "<strong>$namer[0] $namer[1]</strong> total time:";
$time_result = formatTime($r["SUM(time)"]);
...
case "single_student":
	...
	{$graph_query = mysql_query("SELECT DAYNAME(date), SUM(time) AS `numtotal` from cm_case_notes WHERE `date` BETWEEN '$begin' AND '$end' AND `username` = '$_POST[student]' GROUP BY DAYNAME(date) ORDER BY `date` asc");
		...
		$my_array = array_combine($day_list,$time_list_conv);
		...
		$name = $full_name[0] . ' ' . $full_name[1];
			 ...
			 $GphpChart->title = "$name this week: $begin_fmt to $end_fmt"; // this title will be on the chart image
	...
	while ($zz = mysql_fetch_array($graph_query))
		...
		$my_array2 = array_combine($day_list2,$time_list_conv2);
		...
		$name2 = $full_name2[0] . ' ' . $full_name2[1];
		 $GphpChart = new GphpChart('bvs','t'); // 'lc' stands for a line chart 
			 $GphpChart->title = "$name2 this month: $begin_fmt2 to $end_fmt2"; // this title will be on the chart image
	...
	while ($zz = mysql_fetch_array($graph_query))
		...
		$my_array3 = array_combine($int_list,$time_list_conv3);
		...
		$GphpChart = new GphpChart('bvs','t'); // 'lc' stands for a line chart 
		$GphpChart->title = "$name3: $begin_fmt3 to $end_fmt3"; // this title will be on the chart image
	...
	while ($zz = mysql_fetch_array($graph_query))
		...
		$my_array4 = array_combine($int_list2,$time_list_conv4);
		...
		$GphpChart->title = "$name4: $begin_fmt4 to $end_fmt4";
	...
	while ($zz = mysql_fetch_array($graph_query))
		...
		$time_list_conv = scaleValues($time_list);
		$my_array = array_combine($int_list,$time_list_conv);
		$time_array = put_times_in_array($time_list);
		$begin_fmt = formatDate2($begin);
		$end_fmt = formatDate2($end);
		$full_name = getNameAsVar($_POST[student]);
		$name = $full_name[0] . ' ' . $full_name[1];
		$GphpChart = new GphpChart('bvs','t'); // 'lc' stands for a line chart 
		$GphpChart->title = "$name: $begin_fmt to $end_fmt"; // this title will be on the chart image
			 $GphpChart->add_data(array_values($my_array)); // adding values
			 $GphpChart->add_labels('x',array_keys($my_array)); // adding x labels (horizontal axis)
			 $GphpChart->add_labels('x',array_values($time_array)); // adding y labels (left axis)
...
case "all_students":
	...
	$my_array = array_combine($users_array_full,$time_list_conv);
$begin_fmt = formatDate2($begin);
		$end_fmt = formatDate2($end);
		$GphpChart = new GphpChart('p3','t'); 
		$GphpChart->title = "All Your Students: $begin_fmt to $end_fmt"; 
...
echo <<<RESTART
<tr><td align='left'><input type="button" value="New Query" onClick="location.href = 'cm_utilities.php';"></td></table>
<br>
