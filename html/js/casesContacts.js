...
const reloadCaseContacts = async (case_id, searchValue) => {
  ...
  const searchResults = await getCaseContactsData({
    case_id,
    ...(searchValue ? { q: searchValue } : {}),
    update: true,
  });

  contactsContainer.innerHTML = searchResults;
};
// search messagse
live('change', 'contacts_search', async (e) => {
  const searchInput = e.target;
  const searchValue = searchInput.value;
  const case_id = searchInput.dataset.caseid;
  reloadCaseContacts(case_id, searchValue);
});

// clear search
live('click', 'cases_contacts_search_clear', async (e) => {
  const button = getClosest(e.target, '.cases_contacts_search_clear');
  const { caseid: case_id } = button.dataset;
  const searchInput = document.querySelector('.contacts_search');
  searchInput.value = '';
