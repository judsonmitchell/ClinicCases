...
function growRow(target1,target2,toggle)
{
	
	if (toggle == "on")
	{
	$(target1).setStyle({minHeight:'40px',height:''});
	$(target2).setStyle({minHeight:'40px',height:''});
	}
	
	if (toggle == "off")
	{
	$(target1).setStyle({height:'40px'});
	$(target2).setStyle({height:'40px'});
	}
...
<div id = "$line[id]_row" style="width:100%;height:40px;background:url(images/grade_small.jpg) repeat-x;">
<TABLE WIDTH="100%">
<TR title="Click to View Case" alt="Click to View Case" onmouseover="this.style.color='red';this.style.cursor='pointer';growRow('$line[id]_row','$line[id]_cell','on');" onmouseout="this.style.color='black';growRow('$line[id]_row','$line[id]_cell','off');" onClick="location.href='cm_cases.php?direct=$line[case_id]'";><TD WIDTH="15%" STYLE="PADDING-RIGHT:3.5%;" valign="top">
ACTIVITY;
...
ECHO <<<ACTIVITY
</td><td WIDTH="38%"  valign="top"><div id = "$line[id]_cell" style="height:40px;overflow:hidden;">$line[description]</div></td></tr></table>
</div>
