...
<td valign="top" width="15%"><input type="hidden" id="set_by_$e[id]" value="$e[set_by]">
<div style="height:20px;width:100%;text-align:right"><a href="#" alt="Edit this Event" title="Edit this Event" onClick="var checkit = checkAuth('$_SESSION[login]','$_SESSION[class]',$('set_by_$e[id]').value);if (checkit == true){createTargets('add_event','add_event');sendDataGet('event_edit.php?event_id=$e[id]&case_id=$e[case_id]&ieyousuck=$rand');Effect.BlindDown('add_event');return false;} else {return false;};"><img src="images/pencil.png" border="0"></a><a href="#" alt="Delete this Event" title="Delete this Event" onClick="var checkit = checkAuth('$_SESSION[login]','$_SESSION[class]','$e');if (checkit == true){var check = confirm('Are you sure you want to delete all records of this event? If you just want to mark this event as done, click cancel and then click the check on the right.');if (check == true){createTargets('case_activity','case_activity');sendDataGet('event_delete.php?event_id=$e[id]&case_id=$e[case_id]');}else {return false}} else {return false;};"><img src="images/cross.png" border="0" style="margin-left:10px;"></a>
CONTENTS;
if ($e[status] == 'Pending')
{
ECHO <<<SUBCONTENTS
