...
lwRTE.prototype.enable_design_mode = function() {
	...
	var doc = "<html><head>" + base + css + "</head><body " + style + " style='font-family:Times New Roman, Times, serif;font-size:12pt;padding:5px;width:750px'>" + content + "</body></html>";
...
lwRTE.prototype.create_panel = function(title, width) {
	...
	$('.rte-panel', tb).remove();
	var drag, event;
	var left = self.event.layerX;
	var top = self.event.layerY;
