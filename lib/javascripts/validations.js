...
function newCaseValidate(formVals) //cc7
{
    ...
    if(dob.val() !== '')
    {
        ...
        {
            errors.push('<p>Day must be between 1 and 31.</p>');
            dob.addClass('ui-state-error');
        }

        if (parts[2] >  year || parseInt(parts[2]) < 1899)
        {
            errors.push('<p>Year must be between 1899 and ' + year + '.</p>');
            dob.addClass('ui-state-error');
        }
        ...
        if (checker === false)
            {
                errors.push('<p>Months must be in the mm format, e.g 01,02,03 etc.</p>');
                dob.addClass('ui-state-error');
            }
