...
{//start timer
	function getElapsed(startTime)
	{
		...
		timerLoop = setTimeout(function(){getElapsed(startTime)},60000);
	}
...
{//stop timer
	...
	$('#timer_controls').css({'text-align':'left'}).append('<textarea name="description"></textarea><button class="timer_add_button"></button><button class="timer_cancel_add_button"></button>');
	$('#timer_controls textarea').css({'color':'#AAA','width':'60%'}).html('Describe What You Did.').mouseenter(function(){$(this).focus().html('').css({'color':'black'});});
	$('#timer_controls button.timer_add_button').button({icons: {primary: "fff-icon-add"},label: "Add"}).next().button({icons: {primary: "fff-icon-cancel"},label: "Cancel"});
}
...
function timerDestroy()
{
	...
	$.get('html/templates/interior/timer.php',function(data){$('#timer').replaceWith(data);});
...
$(document).ready(function(){
	...
	//Stop timer
	$('#timer_controls button.timer_stop').live('click',function(){
	...
	$('#timer_controls button.timer_add_button').live('click',function(){
		...
		var cseVals = [{name:"csenote_date",value:timerDate},{name:"csenote_seconds",value:elapsed},{name:"csenote_user",value:timerUser},{name:"csenote_case_id",value:caseId},{name:"query_type",value:"add"},{name:"csenote_description",value:description}];

		//Check to see if textarea is valid
		if (description === '' || description == 'Describe What You Did.')
		{notify('<p>Please provide a description of what you did.</p>',true);return false;}
		else
		{
			$.post('lib/php/data/cases_casenotes_process.php', cseVals, function(data) {
				...
				if ($('.case_' + caseId).length)
				{
					...
					$('.case_' + caseId).load('lib/php/data/cases_casenotes_load.php', {'case_id': caseId,'start': '0','update':'yes'})
				}								
            	notify(data)
	...
	$('#timer_controls button.timer_cancel_add_button').live('click',function(){
		 var dialogWin = $('<div class=".dialog-casenote-delete" title="Delete this Time?">You will lose the time you have recorded.  Are you sure?</div>').dialog({
		        autoOpen: true,
		        resizable: false,
		        modal: true,
		        buttons: {
