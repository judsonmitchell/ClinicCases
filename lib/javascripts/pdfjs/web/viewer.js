...
/**
 * Returns the filename from response headers 
 * url {String} The original PDF location.
 * @return {String} Guessed PDF file name.
 * Added by Judson Mitchell for ClinicCases
 */
function getPDFFileNameFromHeaders(header) {
  var test = header.match(/"(.*?)"/);
  var suggestedFilename = test[1];  
  return suggestedFilename || 'document.pdf';
}
...
var SecondaryToolbar = {
  ...
  initialize: function secondaryToolbarInitialize(options) {
    ...
    this.closeWindowButton = options.closeWindowButton;
    ...
    var elements = [
      ...
      { element: this.closeWindowButton,
        handler: this.closeWindowClick },
  ...
  closeWindowClick: function secondaryToolbarPresentationModeClick(evt) {
    //document.getElementById('pdf-viewer').style.display = 'none';
    this.close();
    window.parent.$('#pdf-viewer').trigger('click');
  },
...
var PDFDocumentProperties = (function PDFDocumentPropertiesClosure() {
  ...
  PDFDocumentProperties.prototype = {
    ...
    setHeaderInfo: function PDFDocumentProperties_setHeaderInfo(header) {
      if (header) {
    ...
    _getProperties: function PDFDocumentProperties_getProperties() {
      ...
      this.pdfDocument.getMetadata().then(function(data) {
        var content = {
          'fileName': getPDFFileNameFromHeaders(this.headerInfo),
...
var PDFViewerApplication = {
  ...
  open: function pdfViewOpen(file, scale, password,
    ...
    if (args) {
      PDFViewerApplication.pdfDocumentProperties.setHeaderInfo(args['header']);
    }
  },
  ...
  download: function pdfViewDownload() {
    ...
    var filename = getPDFFileNameFromHeaders(this.header);
...
function webViewerInitialized() {
  ...
  //Custom for CC
  var file = 'item_id' in params ? params.item_id : DEFAULT_URL;
  ...
  document.getElementById('closeWindow').addEventListener('click',
    SecondaryToolbar.closeWindowClick.bind(SecondaryToolbar));
  ...
  if (file && file.lastIndexOf('file:', 0) === 0) {
    ...
    //Custom for ClinicCases
    var url = 'lib/php/data/cases_documents_process.php';
    var ccParams = 'action=open&item_id=' + file + '&doc_type=pdf';
    //
  ...
  if (file) {
    if (params.target === 'board'){
      var url = '../../../php/data/board_process.php';
      var ccParams = 'action=download&item_id=' + file;
    } else {
      var url = '../../../php/data/cases_documents_process.php';
      var ccParams = 'action=open&item_id=' + file + '&doc_type=pdf';
    }
    //
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {//Call a function when the state changes.
        if(xhr.readyState === 4 && xhr.status === 200) {
          //Set a global with the content-disposition file name
          var dispoHeader = xhr.getResponseHeader('Content-disposition');
          PDFViewerApplication.open(new Uint8Array(xhr.response), 0,null,null,{'header': dispoHeader });
        }
    }
    //xhr.onload = function() {
    //  PDFViewerApplication.open(new Uint8Array(xhr.response), 0);
    ...
    //  PDFViewerApplication.open(responseArray, 0);
    //}
  }
}
