...
function update_paths($dbh,$old_path,$new_path,$case_id)
{
	...
	foreach ($paths as $path) {
		...
		$update_folders = $dbh->prepare("UPDATE cm_documents SET folder = $new_path WHERE id = $path[id]");
	...
	foreach ($containers as $container) {
		$part = explode($old_path, $container['containing_folder']);
		$new_container = $new_path . $part[1];

		$update_containers = $dbh->prepare("UPDATE cm_documents SET containing_folder = $new_container WHERE id = $container[id]");
	}
...
if ($action == 'rename')
{
	...
	{
		update_paths($dbh,$container,$new_name,$case_id);//$container is the old path, $new_name is the new path
	}
